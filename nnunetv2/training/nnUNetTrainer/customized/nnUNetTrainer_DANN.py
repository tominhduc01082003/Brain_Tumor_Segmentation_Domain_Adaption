...
class nnUNetTrainerDA(nnUNetTrainer):
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            # initialize grl_alpha_scheduler
    ...
    def build_network_architecture(plans_manager: PlansManager,
        ...
        num_stages = len(configuration_manager.conv_kernel_sizes) # 6

        dim = len(configuration_manager.conv_kernel_sizes[0]) # 3
        conv_op = convert_dim_to_conv_op(dim) # nn.Conv3d

        label_manager = plans_manager.get_label_manager(dataset_json)

        segmentation_network_class_name = 'PlainConvUNet_DAonDecoder'
        mapping = {
    ...
    def configure_optimizers(self):
        ...
        # define Domain Adaptation learning rate scheduler
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            if self.enable_deep_supervision:
                l_s = self.loss(output_S, target,
                            [batch_domain] * len(self._get_deep_supervision_scales()))
            else:
                l_s = self.loss(output_S, target,
                                batch_domain)
    ...
    def validation_step(self, batch: dict) -> dict:
        data = batch['data']
        target = batch['target']
        # Batch Domain from batch.properties
        batch_domain = torch.Tensor([self.domain_mapping[prop['domain']] for prop in batch['properties']]).long().to(
            self.device)
...
# 0/ epoch: default 1000, BS = 4, DA sử dụng epoch = 500
...
# Nếu mà mở enable deep supervision, Segmenter sẽ khá mạnh, vì vậy để da_weight lớn ví dụ 100
...
# 2/ 3 params in GRLalphascheduler: default: p1=0.2,p2=0.7;alpha_max=3

# 4/một số params in Domain Classifier: output_channels = 100
...
class nnUNetTrainerDA_500ep_onEncoder(nnUNetTrainerDA_500epochs):
    ...
    def build_network_architecture(plans_manager: PlansManager,
        ...
        segmentation_network_class_name = 'PlainConvUNet_DA'
        mapping = {
            'PlainConvUNet_DA': PlainConvUNet_DA,
            'PlainConvUNet_DAonDecoder': PlainConvUNet_DAonDecoder,
        }
