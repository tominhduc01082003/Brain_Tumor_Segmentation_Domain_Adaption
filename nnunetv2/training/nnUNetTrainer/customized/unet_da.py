...
class My_UNetDecoder(nn.Module):
    def __init__(self,
        ...
        for s in range(1, n_stages_encoder):
            ...
            # nối input_features_skip với transpconv output (concat input_features_skip with transpconv output)
    ...
    def compute_conv_feature_map_size(self, input_size):
        """
        input_size is the input_size of the encoder!
        :param input_size:
        :return:
        """
...
class DomainClassifier(nn.Module):
    def __init__(self,
        ...
        # the last layer output of encoder as feature
...
class DomainClassifier_onDecoder(nn.Module):
    def __init__(self,
        ...
        input_feature = decoder.output_channels[-(on_ith_decoder + 1)] # number of input dựa theo số stage
        num_convs = 2 # number of conv per block
        output_channels = 100 # conv kernel channel
        domain_channels = 2 # output domain label channel
    ...
    def forward(self, stage_outputs):
        ...
        for stage in range(self.num_convblock - 2): # if num_convblock >2, thêm block doubleConvsMaxPool2(x)
...
class PlainConvUNet_DAonDecoder(nn.Module):
    def __init__(self,
                 ...
                 deep_supervision: bool = False,
                 nonlin_first: bool = False,
                 on_ith_decoder: int = 4,  # domain classifier lấy thứ tự ith stage of decoder output (from [1,2,3,4])
        ...
        # khởi tạo domain classifier ở decoder output: 1~4 for a 5 stages decoder
    ...
    # JR: for each epoch/step, alpha thay đổi trong quá trình huấn luyện
    def forward(self, x):
        skips = self.encoder(x)
